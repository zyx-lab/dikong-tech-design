...
MISSION_INVALID_PARAMS_RESPONSE = business_error_response(
    description="请求体不合法或资源不满足绑定条件，code 固定为 B0001。",
    examples=[
        business_error_example(
            "航线不属于当前租户",
            code="B0001",
        ...
        business_error_example(
            ...
            msg="参数校验失败",
            status_codes=["400"],
            data={"pilot": ["仅允许分配给飞手类型（pilot_operator）"]},
        ),
        business_error_example(
            "提交不可写字段",
            code="B0001",
            msg="参数校验失败",
            status_codes=["400"],
            data={"status": ["该字段在此接口不可写"]},
        ),
        business_error_example(
            "动作接口提交 body",
            code="B0001",
            msg="start 请求不支持提交 body 参数",
            status_codes=["400"],
...
MISSION_STATE_CONFLICT_RESPONSE = business_error_response(
    description="任务当前状态不允许本次流转，code 固定为 C0201 / C0202。",
    examples=[
        business_error_example(
            "已完成任务不可启动",
            code="C0201",
            msg="当前任务状态不允许启动",
            status_codes=["409"],
            data={"mission_id": 101, "status": MissionStatus.COMPLETED},
        ),
        business_error_example(
            "待执行任务不可暂停",
            code="C0201",
            msg="当前任务状态不允许暂停",
            status_codes=["409"],
            data={"mission_id": 102, "status": MissionStatus.PENDING},
        ),
        business_error_example(
            "已取消任务不可恢复",
            code="C0201",
            msg="当前任务状态不允许恢复",
            status_codes=["409"],
            data={"mission_id": 103, "status": MissionStatus.CANCELED},
        ),
    ],
)
...
@extend_schema_view(
    ...
    update=extend_schema(
        summary="全量更新任务",
        description="按任务 ID 全量更新基础字段和绑定资源；不允许在该接口直接修改状态。",
        parameters=[TENANT_CODE_HEADER_PARAMETER],
        request=MissionWriteSerializer,
        responses={
...
class MissionViewSet(
    ...
    def get_queryset(self):
        ...
        # 该接口只做只读检索，不承载任务状态流转编排；返回统一 `code / msg / data`。
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        #
        # 边界与返回语义：
        # 1) 有查看权限且任务存在：返回任务详情快照，code=00000（HTTP 200）；
        # 2) 任务不存在：code=C0404（HTTP 404）；
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        return Response(self._mission_payload(mission), status=status.HTTP_200_OK)
