...
MEDIA_FILE_DELETE_DATA = inline_serializer(
    name="MediaFileDeleteData",
    fields={
        "id": serializers.IntegerField(),
        "is_deleted": serializers.BooleanField(help_text="固定为 true，表示已完成逻辑删除。"),
    },
)
MEDIA_FILE_DELETE_RESPONSE = object_envelope_serializer("MediaFileDeleteResponse", MEDIA_FILE_DELETE_DATA)
...
MEDIA_FILE_INVALID_PARAMS_RESPONSE = business_error_response(
    description="请求体不合法或绑定关系不满足约束，code 固定为 B0001。",
    examples=[
        business_error_example(
            "跨租户飞行记录",
            code="B0001",
            msg="参数校验失败",
            status_codes=["400"],
            data={"flight_record": ["仅允许绑定当前租户下的飞行记录"]},
        ),
        business_error_example(
            "缺少文件名",
            code="B0001",
...
@extend_schema_view(
    ...
    update=extend_schema(
        summary="全量更新媒体文件",
        description="按媒体文件 ID 全量更新元数据字段，不承担逻辑删除恢复或批量操作。",
        parameters=[TENANT_CODE_HEADER_PARAMETER],
        request=MediaFileWriteSerializer,
        responses={
...
class MediaFileViewSet(
    ...
    def get_queryset(self):
        ...
        # 3) 响应由 BusinessApiResponseMixin 统一包装为 `code / msg / data`。
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        #
        # 边界与返回语义：
        # 1) 有查看权限且记录存在：code=00000（HTTP 200）；
        # 2) 记录不存在或已逻辑删除：code=C0404（HTTP 404）；
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        #    返回 code=00000（HTTP 200）；
        # 2) 记录不存在或已逻辑删除：code=C0404（HTTP 404）；
        # 3) 未认证或无删除权限：code=A0401/A0403（HTTP 401/403）。
        instance = self.get_object()
