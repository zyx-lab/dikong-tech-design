...
class DjiGateway:
    ...
    def _current_config(self):
        if self.platform is not None:
            refresh_from_db = getattr(self.platform, "refresh_from_db", None)
            if getattr(self.platform, "pk", None) is not None and callable(refresh_from_db):
                refresh_from_db()
            return self.platform
        return self._session

    def _validate_workspace(self, config) -> bool:
    ...
    def _save_session(
        ...
        *,
        ...
        workspace_config = config or self._session or DjiCloudSession()
        ...
        save = getattr(workspace_config, "save", None)
        if callable(save):
            update_fields = [
                "workspace_id",
                "dji_user_id",
            ...
            if hasattr(workspace_config, "updated_at"):
                update_fields.append("updated_at")
            if getattr(workspace_config, "pk", None) is None:
                workspace_config.save()
            else:
                workspace_config.save(update_fields=update_fields)
        self._session = workspace_config
    ...
    def _request(self, method: str, path: str, *, data: bytes | None, headers: dict[str, str], follow_redirects: bool) -> GatewayResponse:
        ...
        try:
            open_fn = self._urlopen() if opener is None else opener.open
