...
class OpenApiDocsTests(TestCase):
    ...
    def test_business_schema_should_expose_refactored_paths(self):
        ...
        self.assertIn("/api/v1/media-files/{id}/preview-url", paths)
    ...
    def test_business_schema_should_lock_current_operation_surface_and_bodyless_actions(self):
        ...
        expected_methods = {
            ...
            "/api/v1/media-files/{id}/preview-url": {"get"},
    ...
    def test_business_schema_should_expose_media_files_only_on_flight_record_detail(self):
        ...
        self.assertEqual(
            ...
            {"id", "media_type", "file_name", "thumbnail_url", "captured_at", "download_url", "playback_url", "preview_url"},
        )
    ...
    def test_business_schema_should_describe_media_preview_url_response_shape(self):
        response = self.client.get("/api/v1/docs/schema/")
        self.assertEqual(response.status_code, 200)
        schema = response.json()

        operation = self._operation(schema, path="/api/v1/media-files/{id}/preview-url", method="get")
        success_schema = self._resolve_component_schema(
            schema,
            operation["responses"]["200"]["content"]["application/json"]["schema"],
        )
        ...
        self.assertEqual(set(data_schema["properties"].keys()), {"preview_url"})
        self.assertEqual(data_schema["properties"]["preview_url"]["type"], "string")
    ...
    def test_business_schema_should_describe_current_business_error_responses(self):
        ...
        expected_responses = {
            ...
            ("/api/v1/media-files/{id}/preview-url", "get"): {"200", "400", "401", "403", "404", "500"},
