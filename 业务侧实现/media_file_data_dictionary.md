...
**业务规则**：
1. 当前对外查询只返回 `is_deleted=false` 的记录。
2. 当前对外查询只返回已建立 `tenant_media_indexes` 映射的记录。
3. `media_files` 主要由后台同步任务写入，不由前端手工创建。
...
2. 媒体同步优先通过 `job_id -> mission_id` 归属租户；失败时可退回 `device_sn` 反查租户。
3. 下载媒体时，使用 `tenant_media_indexes.dji_file_id` 获取 DJI 下载地址。
