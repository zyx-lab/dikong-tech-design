...
## 本轮增补（2026-03-09）
- 本轮聚焦 API: `POST /api/v1/missions/{id}/start`
- 状态流转语义：
  - 允许 `PENDING -> RUNNING`
  - `RUNNING -> RUNNING` 按幂等成功处理
  - `PAUSED`、`COMPLETED`、`CANCELED`、`FAILED` 不允许 start，返回 `STATE_CONFLICT`
- 输入边界：
  - 请求体必须为空，不接受额外编排参数。
- 作用边界：
  - 仅变更 `missions.status`，不自动处理飞行记录创建、无人机回收或其他跨实体联动。
- 审计语义：
  - 成功启动后写入 `MISSION_START` 审计日志。

## 本轮增补（2026-03-09）
- 本轮聚焦 API: `POST /api/v1/missions/{id}/pause`
- 状态流转语义：
  - 允许 `RUNNING -> PAUSED`
  - `PAUSED -> PAUSED` 按幂等成功处理
  - `PENDING`、`COMPLETED`、`CANCELED`、`FAILED` 不允许 pause，返回 `STATE_CONFLICT`
- 输入边界：
  - 请求体必须为空，不接受额外编排参数。
- 作用边界：
  - 仅变更 `missions.status`，不自动处理飞行记录创建、无人机回收或其他跨实体联动。
